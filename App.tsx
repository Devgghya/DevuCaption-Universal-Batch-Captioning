          ...
          <div className="flex items-center gap-4">
            {selectedProvider === AIProvider.GEMINI && (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all border ${apiKey ? 'bg-green-500/10 text-green-400 border-green-500/30 hover:bg-green-500/20' : 'bg-slate-800 text-slate-400 border-slate-700 hover:text-white hover:border-slate-500'
                  }`}
              >
            ...
            {selectedProvider === AIProvider.GROQ && (
              <button
                onClick={() => setShowApiKeyInput(!showApiKeyInput)}
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all border ${groqApiKey ? 'bg-purple-500/10 text-purple-400 border-purple-500/30 hover:bg-purple-500/20' : 'bg-slate-800 text-slate-400 border-slate-700 hover:text-white hover:border-slate-500'
                  }`}
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div className="max-w-4xl mx-auto text-center mb-12 animate-in fade-in slide-in-from-top-8 duration-700">
          ...
          <div className="mb-6 flex justify-center gap-3 animate-in zoom-in duration-500">
            ...
            <button
              ...
              className={`px-8 py-3 text-sm font-bold rounded-2xl transition-all duration-300 border-2 ${useSamePrompt
                ? 'bg-indigo-600 text-white border-indigo-600 shadow-lg shadow-indigo-500/40 scale-105'
                : 'bg-slate-900 text-slate-400 border-slate-700 hover:text-slate-200 hover:border-slate-500'
                }`}
            >
              ✏️ Same Prompt
          ...
          {!useSamePrompt && (
            <div className="space-y-6 mb-8">
              ...
              <div className="flex justify-center gap-3 animate-in zoom-in duration-500">
                <button
                  ...
                  className={`px-6 py-2.5 text-sm font-bold rounded-xl transition-all duration-300 border-2 ${selectedProvider === AIProvider.GEMINI
                    ? 'bg-green-600 text-white border-green-600 shadow-lg shadow-green-500/40 scale-105'
                    : 'bg-slate-900 text-slate-400 border-slate-700 hover:text-slate-200 hover:border-slate-500'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-6 py-2.5 text-sm font-bold rounded-xl transition-all duration-300 border-2 ${selectedProvider === AIProvider.GROQ
                    ? 'bg-purple-600 text-white border-purple-600 shadow-lg shadow-purple-500/40 scale-105'
                    : 'bg-slate-900 text-slate-400 border-slate-700 hover:text-slate-200 hover:border-slate-500'
                    }`}
                >
                  <span className="flex items-center gap-2">
              ...
              <div className="inline-flex p-1.5 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl">
          ...
          {useSamePrompt && (
            <div className="max-w-3xl mx-auto mb-8 animate-in fade-in slide-in-from-top-4 duration-500">
              <label className="block mb-3 text-sm font-bold text-slate-300">
                Enter your custom caption (will be applied to all images):
              </label>
              <textarea
                value={customPrompt}
                onChange={handleCustomPromptChange}
                placeholder="e.g., A beautiful landscape with mountains and sunset"
                className="w-full bg-slate-900 border-2 border-slate-700 rounded-2xl px-6 py-4 text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors min-h-[120px] resize-y custom-scrollbar shadow-xl"
              />
              <p className="text-xs text-slate-500 mt-2 italic">💡 This exact caption will be used for all uploaded images</p>
            </div>
          )}
        </div>
